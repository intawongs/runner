...
# --- REWARD PAGE (ฉบับเริ่มนับ 05:00 น. และตัดสินตามลำดับเข้าเส้นชัย) ---
elif st.session_state.page == "REWARD":
    st.markdown("<h2 class='center-text'>🎊 สรุปผลการแข่งขัน 🎊</h2>", unsafe_allow_html=True)
    try:
        # 1. ดึงข้อมูล User และ Logs ของเขา
        user = supabase.table("runners").select("*").eq("bib_number", st.session_state.my_bib).single().execute().data
        logs = supabase.table("run_logs").select("*").eq("bib_number", st.session_state.my_bib).execute().data
        scanned = {l['checkpoint_name']: l['scanned_at'] for l in logs}
        
        if user and "Finish" in scanned:
            # 2. การตัดสินอันดับ (Rank) - ใครสแกน Finish ก่อนชนะ (ตัดสินตามลำดับเวลาที่บันทึกจริง)
            # ดึงลำดับการเข้าเส้นชัยแยกตามประเภท (Running/Walking)
            all_finish = supabase.table("run_logs").select("bib_number, scanned_at, runners!inner(race_type)") \
                .eq("checkpoint_name", "Finish") \
                .eq("runners.race_type", user['race_type']) \
                .order("scanned_at", desc=False).execute().data
            
            # หาว่า BIB ของเราอยู่อันดับที่เท่าไหร่ใน List ของคนที่เข้าเส้นชัยแล้ว
            rank = next(i for i, x in enumerate(all_finish) if x['bib_number'] == user['bib_number']) + 1
            has_medal = rank <= 100
            ...
            # คำนวณส่วนต่างเป็นวินาที
            diff_seconds = (finish_time_tz - gun_start).total_seconds()
            
            # แปลงเป็นนาที (ถ้าสแกนก่อน 05.00 ให้เป็น 0)
            total_minutes = int(max(0, diff_seconds) / 60)
            
            # จัดรูปแบบแสดงผล (HH:MM)
            display_time = f"{total_minutes // 60:02d}:{total_minutes % 60:02d} ชม./นาที" if total_minutes >= 60 else f"{total_minutes} นาที"

            final_pic = user['profile_url'] if user['profile_url'] else DEFAULT_AVATAR
            m_uri = f"data:image/jpeg;base64,{get_base64_bin('badge.jpg')}"

            # 4. แสดงผล Card รางวัล
            card_html = f"""
            <div style="font-family: sans-serif; text-align: center; background: white; padding: 25px; border-radius: 20px; border: 6px solid #D4AF37; width: 320px; margin: auto; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">
                ...
                <div style="border-top: 1px dashed #ccc; padding-top: 15px;">
                    <b style="color: {'green' if has_medal else 'red'}; font-size: 18px;">
                        {'🎉 ได้รับเหรียญรางวัล! 🏅' if has_medal else 'ขอบคุณที่ร่วมสนุก!'}
                    </b>
                </div>
            </div>
            """
            ...
            st.warning("⚠️ ข้อมูลยังไม่สมบูรณ์ หรือคุณยังสแกนไม่ถึงจุด Finish")
